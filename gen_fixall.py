...
while 'function SubToggle' in c:
    start = c.find('function SubToggle')
    close = c.find('\n  }\n\n', start)
    if close > 0:
        c = c[:start] + c[close+5:]
    else:
        break
print('Removed old SubToggle definitions')
idx = c.find('  return (\n    <div')
if idx > 0:
    bol = c.rfind('\n', 0, idx) + 1
...
dup = '  function getWordCount() { return (editorRef.current?.innerText || "").replace(/\\s/g, "").length; }\n'
if dup in c:
    c = c.replace(dup, '')
    print('Removed duplicate getWordCount')
